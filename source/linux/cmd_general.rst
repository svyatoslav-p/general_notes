...
+-------------------------------------------------------+-----------------------------------------------------------------------------+
| ``split -a 2 -d -b 4000M sample.iso sample.iso.part`` | Разбить файл sample.iso на части по 4 Гб                                    |
+-------------------------------------------------------+-----------------------------------------------------------------------------+
| ``cat sample.iso.part* > sample.iso``                 | Собрать файл sample.iso из частей sample.iso.part*                          |
+-------------------------------------------------------+-----------------------------------------------------------------------------+
...
|                                                                      Команда                                                                       |                                                                                     Описание                                                                                     |
+====================================================================================================================================================+==================================================================================================================================================================================+
| ``iptables -t nat -A PREROUTING -p udp -s 213.184.251.56,176.213.138.62 -d 95.174.100.68 --dport 5432 -j DNAT --to-destination 192.168.10.4:5432`` | Перенаправить UDP трафик с интерфейса имеющим IP 95.174.100.68 порта 5432 на машину 192.168.10.4 порт 5432, разрешить это только для клиентов с IP 213.184.251.56,176.213.138.62 |
+----------------------------------------------------------------------------------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| ``iptables -t nat -A PREROUTING -p tcp -s 213.184.251.56,176.213.138.62 -d 95.174.100.68 --dport 5432 -j DNAT --to-destination 192.168.10.4:5432`` | Перенаправить TCP трафик с интерфейса имеющим IP 95.174.100.68 порта 5432 на машину 192.168.10.4 порт 5432, разрешить это только для клиентов с IP 213.184.251.56,176.213.138.62 |
+----------------------------------------------------------------------------------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
|                                                                              Команда                                                                              |                                Описание                                |
+===================================================================================================================================================================+========================================================================+
| ``rsync -vP --checksum  --append-verify -e "ssh -p 2206  -i /tmp/Office3/rsa" ubuntu_server.qcow2  router3@213.184.251.56:/home/router3/ubuntu_server.qcow2``     | Копирование файла с проверкой контрольной суммы и возможностью докачки |
+-------------------------------------------------------------------------------------------------------------------------------------------------------------------+------------------------------------------------------------------------+
...
+-------------------------------------------+------------------------------+
| ``virsh vncdisplay NAME``                 | Показать порт VNC            |
+-------------------------------------------+------------------------------+